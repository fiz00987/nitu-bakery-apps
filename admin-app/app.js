...
window.App = (() => {
  ...
  // ── FAST START: serve reads from local cache when offline/slow, and shrink
  // the first paint. The live listeners below refresh the same refs, so the
  // cache only affects startup — never live updates.
  ...
  auth.onAuthStateChanged(user => {
    ...
    if (user) {
      ...
      // Quotes — no tab UI anymore, but keep the list synced so the
...
<div class="card${isOvdPay ? ' overdue-payment' : ''}${statusClass}" id="card-${fk}">
      ...
      ${o.source === 'customer' && o.advance
        ? (o.payShot
          ? `${o.payShotVerified === false
            ? `<div class="overdue-alert">⚠️ স্ক্রিনশট অটো-চেক পাস করেনি — পেমেন্ট যাচাই করে কনফার্ম করুন</div>`
            : (o.payAmountMatch === false ? `<div class="overdue-alert">⚠️ স্ক্রিনশটের টাকার পরিমাণ অগ্রিম/মোটের সাথে মিলছে না — যাচাই করুন</div>` : '')}<div class="card-photo-wrap" style="margin-top:8px"><div class="pay-note">💳 পেমেন্ট স্ক্রিনশট — ট্যাপ করলে বড় হবে</div><img class="card-photo" src="${o.payShot}" onclick="event.stopPropagation();App.openPayShot('${fk}')" alt="পেমেন্ট স্ক্রিনশট" loading="lazy"></div>`
  ...
  // "কোটেশন লিংক" button is emphasized. Fill flavour/weight/price/
  ...
  const setQuoteMode = on => {
    ...
    if (title && !editingId) title.textContent = quoteMode ? '📨 কোটেশন মোড — লিংক তৈরি করুন (১টি কেক)' : 'নতুন অর্ডার';
